// Завдання 1
/**
 * Функція `getDateDetails` отримує об'єкт дати та повертає об'єкт з деякими
 * деталями про дату.
 *
 * date - Об'єкт Date.
 *
...
function getDateDetails(date) {
  // Перевірка, чи є вхідне значення об'єктом Date, це можна зробити перевіривши
  ...
  return {
    ...
    month: date.getMonth() + 1, // треба було добавляти +1? бо в підказці month: 11...
    day: date.getDate(),
  };
}
...
// Завдання 2
/**
 * Функція `setDateDetails` приймає об'єкт Date і рядок ISO дати, і встановлює
 * дату з ISO рядка в об'єкт Date.
 *
 * date - Об'єкт Date, в який потрібно встановити дату.
 * isoString - Рядок дати в форматі ISO.
 *
 * Повертає об'єкт Date з встановленою датою, якщо вхідні дані вірні.
...
function setDateDetails(date, isoString) {
  ...
  // Якщо date не є об'єктом Date, повертаємо рядок
  // "Помилка: вхідне значення має бути об'єктом Date"
  // Перевірка, чи є вхідний ISO рядок валідним, отримавши дату з рядка та
  // перевірити чи є результат числом,
  ...
  let dateFromString = new Date(isoString);
  if (dateFromString.getTime() === NaN) {
    return "Помилка: недійсний ISO рядок";
  }
  date.setTime(dateFromString.getTime());
  return date;
}
...
// Завдання 3
/**
 * Функція `dateToUTC` приймає об'єкт Date та повертає рядок, що представляє дату
 * в UTC форматі.
 *
 * date - Об'єкт Date.
 *
 * Повертає рядок з датою в UTC форматі, якщо вхідне значення є дійсним об'єктом
 * Date. Якщо ні, виводить повідомлення про помилку.
 */
 ...
 *
 * Повертає об'єкт Date з встановленим часом, якщо вхідні дані вірні.
 * Якщо ні, виводить повідомлення про помилку.
 */
...
/**
 * Функція `nextNewYear` приймає об'єкт Date і повертає час до наступного
 * Нового року.
 *
 *
...
function nextNewYear() {
  ...
  let nextNewYear = new Date(currYear + 1, 0, 1);
  let diff = nextNewYear.getTime() - currDate.getTime();
 ...
 * Повертає об'єкт:
 * {
 *   inputDate: // Початкова дата в форматі 'рррр-мм-дд'.
 *   addedDays: // Кількість доданих днів.
 *   resultDate: // Результуюча дата в форматі 'рррр-мм-дд'.
 * }
 */
function addDays(date, days) {
  ...
  // Повертаємо об'єкт з початковою датою, кількістю доданих днів та
  // результуючою датою.
...
function getDaysInMonth(date) {
  ...
  // Віднімання одного дня від наступного місяця, щоб отримати останній день
  // поточного місяця.
  ...
  let nextMonth = new Date(currYear, currMonth + 1, 1);
  nextMonth.setDate(nextMonth.getDate() - 1);
  return { inputDate: inputDate, daysInMonth: nextMonth.getDate() };
}
 ...
 * Повертає об'єкт:
 * {
 *   birthDate: // Дата народження в форматі 'рррр-мм-дд'.
 *   age: // Обчислений вік.
 * }
 */
function getAge(birthDate) {
  ...
  let savedBirthDate = birthDate.toISOString();
 ...
 * Повертає об'єкт:
 * {
 *   date1: // Перша дата в форматі 'рррр-мм-дд'.
 *   date2: // Друга дата в форматі 'рррр-мм-дд'.
 *   comparison: // Результат порівняння: -1, якщо date1 < date2,
...
function compareDates(date1, date2) {
  ...
  // "Помилка: вхідне значення має бути об'єктом Date"
  // Порівняння дат і отримання результату порівняння: -1,
  // якщо date1 < date2, 0, якщо date1 === date2, 1, якщо date1 > date2.
  // Збереження дат для виведення в форматі ISO.
  // Повертаємо об'єкт з порівнюваними датами та результатом порівняння.

  if (typeof date1.getTime !== "function" && date2.getTime !== "function") {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }
  if (date1 < date2) {
    return -1;
  ...
  let savedDate1 = date1.toISOString();
  let savedDate2 = date2.toISOString();

  return {
    date1: savedDate1,
    date2: savedDate2,
    comparison: resultDate,
  };
}
 ...
 * Повертає об'єкт:
 * {
 *   startDate: // Початкова дата в форматі 'рррр-мм-дд'.
 *   endDate: // Кінцева дата в форматі 'рррр-мм-дд'.
...
function getDaysDifference(startDate, endDate) {
  ...
  // Перетворення різниці в мілісекундах у дні поділивши мілісекунди на
  // (1000 * 60 * 60 * 24).
  // Збереження дат для виведення в форматі ISO.
  ...
  if (
    ...
    endDate.getTime !== "function"
  ) {
    return "Помилка: вхідне значення має бути об'єктом Date";
  }

  const msStartDate = startDate.getTime();
  const msEndtDate = endDate.getTime();

  const diff = msEndtDate - msStartDate;
